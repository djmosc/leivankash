...
class Press extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if ( $custom_query->have_posts() ) :
			...
			while ( $custom_query->have_posts() ) : $custom_query->the_post();
			?>
				<h2 class="title"><?php _e("Latest Press", THEME_NAME); ?></h2>
				<a href="<?php echo get_permalink(get_field('press_page', 'options')); ?>" class="post press-release overlay-btn" >
					<div class="featured-image">
						<?php the_post_thumbnail('press_release_widget', array('class' => 'thumbnail scale')); ?>
					</div>
				</a>
			<?php
			$i++;
